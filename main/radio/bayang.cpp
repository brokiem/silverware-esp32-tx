...
void bayang_build_data_packet(uint8_t* packet, const struct BayangControlState* state) {
    ...
    packet[2] = 0;
    if (state->aux_flip) packet[2] |= BAYANG_FLAG_FLIP;
    if (state->aux_rth) packet[2] |= BAYANG_FLAG_RTH;
    if (state->aux_headless) packet[2] |= BAYANG_FLAG_HEADLESS;
    if (state->aux_picture) packet[2] |= BAYANG_FLAG_PICTURE;
    if (state->aux_video) packet[2] |= BAYANG_FLAG_VIDEO;
    
    packet[3] = 0;
    if (state->aux_take_off) packet[3] |= BAYANG_FLAG_TAKE_OFF;
    if (state->aux_inverted) packet[3] |= BAYANG_FLAG_INVERTED;
    if (state->aux_emg_stop) packet[3] |= BAYANG_FLAG_EMG_STOP;
    
    bool headless = state->aux_headless;
    
    packet[4] = (state->roll >> 8) + (headless ? 0x7C : dynamic_trim(state->roll));
    packet[5] = state->roll & 0xFF;
    
    packet[6] = (state->pitch >> 8) + (headless ? 0x7C : dynamic_trim(state->pitch));
    packet[7] = state->pitch & 0xFF;
    
    packet[8] = (state->throttle >> 8) + 0x7C;
    packet[9] = state->throttle & 0xFF;
    
    packet[10] = (state->yaw >> 8) + (headless ? 0x7C : dynamic_trim(state->yaw));
    packet[11] = state->yaw & 0xFF;

...
void feedback_task(void*) {
    ...
    for (;;) {
        ...
        write_active_level(PIN_BUZZER, output.buzzerOn, BUZZER_ACTIVE_HIGH);
...
bool feedback_init(SystemState initial_state) {
    ...
    write_active_level(PIN_BUZZER, false, BUZZER_ACTIVE_HIGH);
    __atomic_store_n(&requested_transition, pack_transition(STATE_BOOT, initial_state), __ATOMIC_RELEASE);
    return xTaskCreatePinnedToCore(feedback_task, "feedback", 2048, nullptr, 1, nullptr, 0) == pdPASS;
}
